...
export default class Version {
	...
	private static readonly log = Logger.getLogger('FurniturePack', 'Version');
	...
	private static onWorldLoad(): void {
		...
		this.log.info('FurniturePack is running!');
	}
...
export class VersionCommand implements CustomCommand {
	readonly name = 'xp_furniture:version';
	readonly description =
		'Displays the current version of the XP Furniture Add-On.';
	readonly permissionLevel = CommandPermissionLevel.Any;

	@BindThis
	run(_origin: CustomCommandOrigin): CustomCommandResult {
		const msgBody =
			`Furniture Pack is running on ${Version.get().version}! ` +

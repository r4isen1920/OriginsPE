...
/**
 * Gift of the Winds is an active power that allows Elytrians to launch themselves into the air 
 * by sneaking and jumping simultaneously. 
...
export class GiftOfTheWinds implements Power {
	...
	private static readonly log = Log.get('GiftOfTheWinds');
	...
	static onPlayerTick(player: Player): void {
		try {
			...
			GiftOfTheWinds.log.error(
				`Error inside GiftOfTheWinds activation handler: ${error?.stack ?? error}`
			);
		}
	}
}

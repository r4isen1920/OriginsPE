import { Player, TicksPerSecond, EntityHitEntityAfterEvent, EntityComponentTypes, system, world, Entity, Vector3 } from '@minecraft/server';
...
export class Bloodthirsty implements Power {
    ...
    private static readonly DIMENSIONS = ['minecraft:overworld', 'minecraft:nether', 'minecraft:the_end'];

    private static bloodPercent(blood: number): number {
        return Math.round((Math.max(0, Math.min(Bloodthirsty.MAX_BLOOD, blood)) / Bloodthirsty.MAX_BLOOD) * 99);
    }
    ...
    if (elapsed >= Bloodthirsty.DECAY_INTERVAL_TICKS) {
        ...
        if (blood <= 0) {
            ...
            if (!state.isOnCooldown(Bloodthirsty.STARVE_DAMAGE_KEY, now)) {
                ...
                state.setCooldown(Bloodthirsty.STARVE_DAMAGE_KEY, now, Bloodthirsty.STARVE_DAMAGE_INTERVAL_TICKS);
            }
        }
    }

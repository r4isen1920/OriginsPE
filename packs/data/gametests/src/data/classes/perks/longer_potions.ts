import { ItemStack, world, system, TicksPerSecond, type Player } from "@minecraft/server";
 ...
 */
export function main(player: Player, potion_filter: "_long" | "_potent", itemList: PotionItem[]) {
  ...
  for (const item of potionItemsInInventory) {
    const convertItem = new ItemStack(
      item.item.typeId.replace(
        "r4isen1920_originspe:temp_",
        "r4isen1920_originspe:cleric_",
      ),
      item.item.amount,
    );
    ...
    itemData.effects.forEach((effect) => {
      const effectName =
        (effect.isNegativeEffect ? "§c" : "§7") +
        effect.name.replace(/.+_of_|_/gm, " ").toTitle();
      const amplifier = potency[effect.amplifier] || effect.amplifier;
      const duration =
        Math.floor(effect.duration / 60) +
        ":" +
        effect.duration.toString().padStart(2, "0");
    ...
    if (itemData.whenApplied.length > 0)
      setLore.push("§r§7", "§r§5When Applied:", ...itemData.whenApplied);
    setLore.push("§r§7", "§r§6Enhanced Potion§r");
    convertItem.setLore(setLore);

    const slotType = item.slot <= 8 ? "hotbar" : "inventory";
    const potionType = item?.item?.typeId?.includes("splash")
      ? "splash_potion"
...
system.runTimeout(() => {
  world.afterEvents.itemCompleteUse.subscribe((event) => {
    ...
    const itemList = itemStack.typeId.includes("_long")
      ? items
      : powerful_potions.items;
    const itemData = itemList.find((i) => itemStack.typeId.includes(i.typeId));
    if (!itemData) return;

    itemData.effects.forEach((effect) => {
      source.addEffect(effect.name, Math.max(TicksPerSecond * effect.duration, 1), {
...
const items: PotionItem[] = [
  {
    ...
    dataValue: 13,
    effects: [{ name: "fire_resistance", duration: 960, amplifier: 0, isNegativeEffect: false }],
    whenApplied: [],
  },
  {
    typeId: "lingering_potion_of_invisibility",
    dataValue: 8,
    effects: [{ name: "invisibility", duration: 960, amplifier: 0, isNegativeEffect: false }],
    whenApplied: [],
  },
  {
    typeId: "lingering_potion_of_leaping",
    dataValue: 10,
    effects: [{ name: "jump_boost", duration: 960, amplifier: 0, isNegativeEffect: false }],
    whenApplied: [],
  },
  {
    typeId: "lingering_potion_of_night_vision",
    dataValue: 6,
    effects: [{ name: "night_vision", duration: 960, amplifier: 0, isNegativeEffect: false }],
    whenApplied: [],
  },
  {
    typeId: "lingering_potion_of_poison",
    dataValue: 26,
    effects: [{ name: "poison", duration: 180, amplifier: 0, isNegativeEffect: true }],
    whenApplied: [],
  },
  {
    typeId: "lingering_potion_of_regeneration",
    dataValue: 29,
    effects: [{ name: "regeneration", duration: 240, amplifier: 0, isNegativeEffect: false }],
    whenApplied: [],
  },
  {
    typeId: "lingering_potion_of_slow_falling",
    dataValue: 41,
    effects: [{ name: "slow_falling", duration: 180, amplifier: 0, isNegativeEffect: false }],
    whenApplied: [],
  },
  {
    typeId: "lingering_potion_of_slowness",
    dataValue: 18,
    effects: [{ name: "slowness", duration: 480, amplifier: 0, isNegativeEffect: true }],
    whenApplied: ["§r§c-15% Speed§r"],
  },
  {
    typeId: "lingering_potion_of_strength",
    dataValue: 32,
    effects: [{ name: "strength", duration: 960, amplifier: 0, isNegativeEffect: false }],
    whenApplied: [],
  },
  {
    typeId: "lingering_potion_of_swiftness",
    dataValue: 15,
    effects: [{ name: "speed", duration: 960, amplifier: 0, isNegativeEffect: false }],
    whenApplied: ["§r§9+20% Speed§r"],
  },
  {
    ...
    whenApplied: ["§r§c-60% Speed§r"],
  },
  {
    typeId: "lingering_potion_of_water_breathing",
    dataValue: 20,
    effects: [{ name: "water_breathing", duration: 960, amplifier: 0, isNegativeEffect: false }],
    whenApplied: [],
  },
  {
    typeId: "lingering_potion_of_weakness",
    dataValue: 35,
    effects: [{ name: "weakness", duration: 480, amplifier: 0, isNegativeEffect: true }],
    whenApplied: ["§r§c-0.7 Attack Damage§r"],
  },
  {

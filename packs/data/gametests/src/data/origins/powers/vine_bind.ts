...
system.runTimeout(() => {
  world.afterEvents.entityHitEntity.subscribe((event) => {
    ...
    if (
      damagingEntity.hasTag("power_vine_bind") &&
      !damagingEntity.hasTag("cooldown_24")
    ) {
      ...
      const chainContext: ChainContext = {
        ...
        entityInfos: [
          { entity: hitEntity, tagTimeoutId: hitEntityTagTimeoutId },
        ],
        vineLinkInfos: [],
      };
  ...
  world.afterEvents.entityHurt.subscribe((event) => {
    ...
    if (
      !hurtEntity?.isValid ||
      damageSource.cause !== EntityDamageCause.magic
    ) {
      return;
    }
...
function propagateChain(
  currentSourceEntity: Entity,
  chainContext: ChainContext,
  linksMadeCount: number,
): void {
  ...
  if (linksMadeCount >= MAX_CHAIN_LINKS) {
    ...
    startCooldown(player as Player);
    return;
  }
  ...
  system.runTimeout(() => {
    ...
    const potentialTargets =
      currentSourceEntity.dimension.getEntities(searchOptions);

    let nextTargetEntity: Entity | null = null;
    let minDistanceSq = Infinity;

    for (const entity of potentialTargets) {
      if (
        !entity.isValid ||
        entity.id === currentSourceEntity.id ||
        chainContext.entityInfos.some((ei) => ei.entity.id === entity.id)
      ) {
        continue;
      }

      const sourceLocationRaw = currentSourceEntity.getHeadLocation();
      const sourceLocation: Vector3 = {
      ...
      if (distance > 0) {
        const hitBlock = currentSourceEntity.dimension.getBlockFromRay(
          sourceLocation,
          direction,
          {
        ...
        if (hitBlock && !ignoredBlockTypes.includes((hitBlock as any).typeId)) {
          continue;
        }
      }
    ...
    if (nextTargetEntity && nextTargetEntity.isValid) {
      const targetEntity = nextTargetEntity;
      targetEntity.addTag(playerSpecificTag);

      const nextTargetTagTimeoutId = system.runTimeout(() => {
        if (targetEntity.isValid) {
          targetEntity.removeTag(playerSpecificTag);
        }
      }, VINE_EFFECT_DURATION_TICKS + CHAIN_PROPAGATION_DELAY_TICKS);
      chainContext.entityInfos.push({
        entity: targetEntity,
        tagTimeoutId: nextTargetTagTimeoutId,
      });

      const vineSpawnData = spawnVine(
        currentSourceEntity,
        targetEntity,
        linksMadeCount === 0,
        world.getEntity(chainContext.chainOwnerId) as Player,
      );
      ...
      propagateChain(targetEntity, chainContext, linksMadeCount + 1);
    } else {
      startCooldown(player as Player);
    }
...
function spawnVine(
  from: Entity,
  to: Entity,
  isInitialSourceInChain: boolean,
  owner: Player,
): { vineEntity?: Entity } {
  ...
  const locationFrom: Vector3 = {
    x: rawlocationFrom.x,
    y: rawlocationFrom.y + yDifferenceFrom,
    z: rawlocationFrom.z,
  };
  const locationTo: Vector3 = {
    x: rawlocationTo.x,
  ...
  let adjustedLocationTo: Vector3 = { ...locationTo };

  if (magnitude > 0) {
    const unitVector: Vector3 = {
  ...
  (world as any).playSound("totem_shield.break", locationFrom, {
    volume: 0.5,
    pitch: 0.5,
  });
  ...
  vineEntity.setProperty(
    "r4isen1920_originspe:length",
  ...
  if (particleCount > 0) {
    ...
    for (let i = 0; i <= 1; i += particleStep) {
      ...
      from.dimension.spawnParticle(
        "r4isen1920_originspe:rootkin_vine_spawn",
        particlePos,
      );
      system.runTimeout(() => {
        if (!from.isValid) return;
        from.dimension.spawnParticle(
          "r4isen1920_originspe:rootkin_vine_despawn",
          particlePos,
        );
      }, VINE_EFFECT_DURATION_TICKS);
    }
...
function triggerChainCollapse(chainContext: ChainContext): void {
  system.runTimeout(() => {
    let totalHealth = 0;
    const entitiesToDamage: Entity[] = [];

    for (const entityInfo of chainContext.entityInfos) {
      const entity = entityInfo.entity;
      if (entity?.isValid) {
        const healthComponent: any = entity.getComponent(
    ...
    for (const entity of entitiesToDamage) {
      if (entity.isValid) {
        ...
        (world as any).playSound("totem_shield.deactivate", entity.location);
        (chainOwner as Player)?.playSound("totem_shield.deactivate");
      }
    }

    incrementWrathbloomStack(
      world.getEntity(chainContext.chainOwnerId) as Player,
    );
  }, MAX_CHAIN_LINKS * CHAIN_PROPAGATION_DELAY_TICKS);
}

function startCooldown(player: Player): void {
  system.runTimeout(() => {
    new ResourceBar(24, 0, 100, 6, false).push(player);
  }, TicksPerSecond * 2.5);
}

...
function sacrifice_stinger(player: Player): void {
  if (!player.hasTag("power_sacrifice_stinger")) return;

  const stingerTags = player
    .getTags()
    .filter((tag) => tag.includes("stinger_level_") && !tag.includes("_was"));
  ...
  switch (true) {
    case player.hasTag("stinger_level_7"):
      if (
        player.hasTag("_was_stinger_level_6") ||
        player.hasTag("_was_stinger_level_0")
      ) {
        onIncrementStingerLevel(player, 7);
        break;
      }
    ...
    case player.hasTag("stinger_level_6"):
      if (player.hasTag("_was_stinger_level_7")) {
        onDecrementStingerLevel(player, 6);
        break;
      } else if (player.hasTag("_was_stinger_level_5")) {
        onIncrementStingerLevel(player, 6);
        break;
      }
    ...
    case player.hasTag("stinger_level_5"):
      if (player.hasTag("_was_stinger_level_6")) {
        onDecrementStingerLevel(player, 5);
        break;
      } else if (player.hasTag("_was_stinger_level_4")) {
        onIncrementStingerLevel(player, 5);
        break;
      }
    ...
    case player.hasTag("stinger_level_4"):
      if (player.hasTag("_was_stinger_level_5")) {
        onDecrementStingerLevel(player, 4);
        break;
      } else if (player.hasTag("_was_stinger_level_3")) {
        onIncrementStingerLevel(player, 4);
        break;
      }
    ...
    case player.hasTag("stinger_level_3"):
      if (player.hasTag("_was_stinger_level_4")) {
        onDecrementStingerLevel(player, 3);
        break;
      } else if (player.hasTag("_was_stinger_level_2")) {
        onIncrementStingerLevel(player, 3);
        break;
      }
    ...
    case player.hasTag("stinger_level_2"):
      if (player.hasTag("_was_stinger_level_3")) {
        onDecrementStingerLevel(player, 2);
        break;
      } else if (player.hasTag("_was_stinger_level_1")) {
        onIncrementStingerLevel(player, 2);
        break;
      }
    ...
    case player.hasTag("stinger_level_1"):
      if (player.hasTag("_was_stinger_level_2")) {
        onDecrementStingerLevel(player, 1);
        break;
      } else if (player.hasTag("_was_stinger_level_0")) {
        onIncrementStingerLevel(player, 1);
        break;
      }
...
export function changeStingerLevel(
  player: Player,
  level: string | number,
): void {
  const levelClamped = Math.clamp(level as number, 0, 7);
...
function onIncrementStingerLevel(player: Player, level: string | number): void {
  ...
  new ResourceBar(
    13,
    BAR_LEVELS[(level as number) - 1],
    BAR_LEVELS[level as number],
    1,
    true,
  ).push(player);
}

function onDecrementStingerLevel(player: Player, level: string | number): void {
  removeTags(player, "_was_stinger_level");

...
export async function initModules(player: Player): Promise<void> {
  ...
  const playerOrigin =
    player
      .getTags()
      .find((tag) => tag.startsWith("race_"))
      ?.replace("race_", "") || "human";
  const playerClass =
    player
      .getTags()
      .find((tag) => tag.startsWith("class_"))
      ?.replace("class_", "") || "nitwit";
  ...
  if (
    player.hasTag("power_phantomize") &&
...
function loadPlayerEffects(
  ...
  try {
    ...
    console.warn(
      `[r4isen1920][OriginsPE] Failed to load ${type} effect: '${value}' for ${player.name}`,
    );
    console.warn(`[r4isen1920][OriginsPE] ${e}`);
  }
}

export async function importOriginsModule(
  path: string,
  errMsg: string,
): Promise<object | null> {
  try {
...
system.afterEvents.scriptEventReceive.subscribe(
  (event) => {
    ...
    if (
      id !== "r4isen1920_originspe:player" ||
      !sourceEntity ||
      message.length === 0
    )
      return;
